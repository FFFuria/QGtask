...
void quickSort(int* data, int L, int R) {
    ...
    while (left < right) {
        while (left < right && data[right] >= pivotValue) right--;
...
void checkPerformance(void (*sorter)(int*, int), int* origin, int size, char* name) {
    ...
    memcpy(numbers, origin, size * sizeof(int));
...
void testCountSort(int* origin, int size) {
    ...
    clock_t start = clock();
    int* result = countSort(numbers, size);
    double duration = (clock() - start) * 1000.0 / CLOCKS_PER_SEC;
...
void testQuickSortNonRecursive(int* origin, int size) {
    ...
    clock_t start = clock();
    QuickSortNonRecursive(numbers, size);
    double duration = (clock() - start) * 1000.0 / CLOCKS_PER_SEC;

    printf("[非递归快速排序] 耗时: %.8f毫秒\n", duration);
...
void testQQuickSortRecursive(int* origin, int size) {
    ...
    clock_t start = clock();
    QuickSortWrapper(numbers, size);
    double duration = (clock() - start) * 1000.0 / CLOCKS_PER_SEC;

    printf("[递归快速排序] 耗时: %.8f毫秒\n", duration);
    
    free(numbers);
}

int main() {
    int testSizes[] = {10000, 50000, 200000};  
    srand(time(0));
    ...
    for (int i = 0; i < sizeof(testSizes)/sizeof(int); i++) {
        ...
        checkPerformance(insertion, numbers, size, "插入");
        checkPerformance(runQuickSort, numbers, size, "快速");
        testCountSort(numbers, size);
        checkPerformance(digitSort, numbers, size, "基数");
        checkPerformance(mergeSort, numbers, size, "归并");
        checkPerformance(NewBubbleSort, numbers, size, "优化冒泡");
        testQuickSortNonRecursive(numbers, size);
        testQQuickSortRecursive(numbers, size);
